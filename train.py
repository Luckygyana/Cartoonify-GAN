...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.sizes = [self.input_size - 32,
                      self.input_size, self.input_size + 32]
        ...
        if not self.ignore_vgg:
            ...
            self.vgg = tf.keras.Model(
                inputs=base_model.input, outputs=tmp_vgg_output)
        ...
        self.g_total_loss_metric = tf.keras.metrics.Mean(
            "g_total_loss", dtype=tf.float32)
        self.g_adv_loss_metric = tf.keras.metrics.Mean(
            "g_adversarial_loss", dtype=tf.float32)
        if self.content_lambda != 0.:
            self.content_loss_metric = tf.keras.metrics.Mean(
                "content_loss", dtype=tf.float32)
        if self.style_lambda != 0.:
            self.style_loss_metric = tf.keras.metrics.Mean(
                "style_loss", dtype=tf.float32)
        self.d_total_loss_metric = tf.keras.metrics.Mean(
            "d_total_loss", dtype=tf.float32)
        self.d_real_loss_metric = tf.keras.metrics.Mean(
            "d_real_loss", dtype=tf.float32)
        self.d_fake_loss_metric = tf.keras.metrics.Mean(
            "d_fake_loss", dtype=tf.float32)
        self.d_smooth_loss_metric = tf.keras.metrics.Mean(
            "d_smooth_loss", dtype=tf.float32)
    ...
    def get_dataset(self, dataset_name, domain, _type, batch_size):
        files = glob(os.path.join(
            self.data_dir, dataset_name, f"{_type}{domain}", "*"))
    ...
    def discriminator_loss(self, real_output, fake_output, smooth_output):
        real_loss = self.discriminator_loss_object(
            tf.ones_like(real_output), real_output)
        fake_loss = self.discriminator_loss_object(
    ...
    def train_step(self, source_images, target_images, smooth_images,
        ...
        with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:
            ...
            g_adv_loss = self.g_adv_lambda * self.generator_adversarial_loss(
                fake_output)
            ...
            if self.content_lambda != 0. or self.style_lambda != 0.:
                ...
                if self.style_lambda != 0.:
                    s_loss = self.style_lambda * self.style_loss(
                        self.pass_to_vgg(
                            target_images[:vgg_generated_images.shape[0]]),
                        vgg_generated_images)
                    g_total_loss = g_total_loss + s_loss

        d_grads = d_tape.gradient(
            d_total_loss, discriminator.trainable_variables)
    ...
    def pretrain_generator(self):
        summary_writer = tf.summary.create_file_writer(
            os.path.join(self.log_dir, "pretrain"))
        self.logger.info(
            f"Starting to pretrain generator with {self.pretrain_epochs} epochs...")
        self.logger.info(
        ...
        generator = Generator(
            base_filters=2 if self.debug else 64, light=self.light)
        ...
        self.logger.info(
            f"Try restoring checkpoint: `{self.pretrain_checkpoint_prefix}`...")
        try:
        ...
        for epoch in range(epochs):
                ...
                if step % self.pretrain_reporting_steps == 0:
                    ...
                    with summary_writer.as_default():
                        ...
                        if not self.disable_sampling:
                            ...
                            tf.summary.image(
                                'pretrain_generated_images', img, step=global_step)
            ...
            with summary_writer.as_default():
                if not self.disable_sampling:
                    ...
                    tf.summary.image(
                        'pretrain_val_generated_images', img, step=epoch)

            if epoch % self.pretrain_saving_epochs == 0:
                self.logger.info(
                    f"Saving checkpoints after epoch {epoch_idx} ended...")
    ...
    def train_gan(self):
        ...
        ds_smooth, _ = self.get_dataset(dataset_name=self.dataset_name,
                                        ...
                                        batch_size=self.batch_size)
        self.logger.info(
            "Setting up optimizer to update generator and discriminator...")
        g_optimizer = tf.keras.optimizers.Adam(
            learning_rate=self.generator_lr, beta_1=.5)
        d_optimizer = tf.keras.optimizers.Adam(
            learning_rate=self.discriminator_lr, beta_1=.5)
        if self.multi_scale:
            self.logger.info(f"Initializing generator with "
                             f"batch_size: {self.batch_size}, input_size: multi-scale...")
        else:
        ...
        self.logger.info(
            f"Searching existing checkpoints: `{self.generator_checkpoint_prefix}`...")
        try:
        ...
        try:
            ...
            self.logger.info(
                "specified checkpoint is not found, training from scratch...")
        ...
        for epoch in range(epochs):
                ...
                if step % self.reporting_steps == 0:
                    ...
                    with summary_writer.as_default():
                        for metric, name in self.metric_and_names:
                            tf.summary.scalar(
                                name, metric.result(), step=global_step)
                            metric.reset_states()
                        if not self.disable_sampling:
                            fake_batch = tf.cast(
                                (generator(real_batch, training=False) + 1) * 127.5, tf.uint8)
                            ...
                            tf.summary.image(
                                'gan_generated_images', img, step=global_step)
            ...
            with summary_writer.as_default():
                if not self.disable_sampling:
                    ...
                    tf.summary.image(
                        'gan_val_generated_images', img, step=epoch)
            self.logger.info(
                f"Saving checkpoints after epoch {epoch_idx} ended...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gan_type", type=str,
                        default="lsgan", choices=["gan", "lsgan"])
    ...
    parser.add_argument("--generator_checkpoint_prefix",
                        type=str, default="generator")
    parser.add_argument("--discriminator_checkpoint_prefix",
                        type=str, default="discriminator")
    parser.add_argument("--pretrain_checkpoint_prefix",
                        type=str, default="pretrain_generator")
    ...
    parser.add_argument("--discriminator_name", type=str,
